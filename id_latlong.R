...
#First.candidate.ID is the column label for first candidate ligation ID in cervus csv file, First.ligation_id is the name of the column in lab1 I want to join with
...
idcsv <- merge(idcsv, lab3, by.x = "Offspring.ID", by.y = "Offs.ligation_id", all.x = T)
...
write.csv(idcsv, file = paste("identity/", "_idanalyis.csv", sep = ""), row.names = F)
write.table(idcsv, file = "idanal.csv")

write.csv(idcsv ,idanalysis, sep=',', row.names = FALSE) 
write.xlsx(idcsv, idanalysis.xlsx, sheetName="Sheet1")
