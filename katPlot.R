katPlot <- function (x, axes = FALSE, add = FALSE, xlim = c(-1, 1), ylim = c(-1, 
    ...
    alpha = 1), mark.expand = 15, ...) 
{
    ...
    if (length(loops.e) > 0) {
        ...
        loop <- function(x0, y0, cx = x0, cy = y0, color, angle = 0, 
            ...
            rad <- angle
            center <- c(cx, cy)
			x0_adj <- diff(par()$usr[3:4])*0.1 ; #c(x0+x0_adj[1],x0+x0_adj[2])
			y0_adj <- diff(par()$usr[1:2])*0.1
			cp <- matrix(c(x0, y0, x0 + x0_adj, y0 + y0_adj, x0 + x0_adj, y0 - y0_adj, x0, y0), ncol = 2, byrow = TRUE)
			#cp <- matrix(c(x0, y0, x0 + 0.1, y0 + 0.05, x0 + 0.1, y0 - 0.05, x0, y0), ncol = 2, byrow = TRUE) 
