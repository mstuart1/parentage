...
#create one column list
one <- all1 %>% ungroup() %>% select(one)
two <- all1 %>% ungroup() %>% select(two)
two <- rename(two, one=two)
ids <- bind_rows(one, two)

#eliminate repeats
finalids <- ids %>% group_by(one) %>% filter(row_number() == 1)
dim(finalids)
#write out the resulting IDs into a txt file
write.table(finalids, file="matchIDspedigree.txt", sep=" ")